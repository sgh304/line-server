...
# to test ended with the line number, as well as one of those text file's specific length.
...
for line in [0, 1, 2, 3000, 30505, 23380915]:
    assert(requests.get(make_line_url(line)).text.split(' ')[-1] == str(line))
...
assert(requests.get('http://localhost:3000/').text == 'Welcome to Sam\'s line server. Currently serving lines from testfile4.txt.')
assert(requests.get('http://localhost:3000/lines/').text == 'testfile4.txt contains 23380916 lines.')